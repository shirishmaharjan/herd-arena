...
export default function HerdArenaFinalMaster() {
  ...
  const resolveTeam = (slot: string) => {
    ...
    // Explicit check for Semi-Final Losers (m101/m102) to avoid "Group L" conflicts
          ...
          <div className="p-12 md:w-1/2 flex flex-col justify-center items-center text-center bg-white/80">
             ...
             <div className="space-y-4 w-full max-w-xs">
                <div className="relative">
                   ...
                   <input type="text" placeholder="Full Legal Name" className="w-full pl-12 pr-4 py-4 rounded-2xl border-2 border-slate-100 bg-slate-50/50 outline-none font-bold text-lg focus:bg-white focus:border-blue-600 transition-all" value={bracketName} onChange={(e) => setBracketName(e.target.value)} />
                </div>
            ...
            <section className="flex gap-10 overflow-x-auto pb-20 no-scrollbar items-start">
               ...
               <div className="flex-shrink-0 pt-[450px] space-y-40">
                  <div className="w-64 p-8 bg-amber-50 border-2 border-amber-100 rounded-[3rem] shadow-sm text-center">
                     <h3 className="text-[10px] font-black text-amber-700 uppercase tracking-widest mb-4 italic italic text-center">3rd Place Match</h3>
      ...
      {[t1, t2].map((t, i) => (
        <button key={i} disabled={t?.placeholder} onClick={() => onPick(t)} className={`w-full text-left p-4 flex justify-between items-center border-b last:border-0 border-slate-50 transition-all ${winner?.id === t?.id ? 'bg-blue-600 text-white font-black' : 'hover:bg-blue-50'}`}>
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <span className={`text-[10px] font-black uppercase tracking-tight truncate ${t?.placeholder ? 'text-slate-300 italic font-medium' : 'text-slate-700'}`}>{t?.n || t?.placeholder}</span>
          </div>
