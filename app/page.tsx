...
import { Trophy, Check, Award, Info, ShieldCheck, Save, ArrowRight, Database, Star, Search, Users, Loader2 } from 'lucide-react';
...
export default function HerdArenaFinalMaster() {
  ...
  const submitToDatabase = async () => {
    if (!isAdmin && selectedThirdsIds.length < 8) return alert("Select exactly 8 best 3rd-place teams!");
    
    const table = isAdmin ? 'official_results' : 'submissions';
    const payload = {
      bracket_name: isAdmin ? 'OFFICIAL_KEY' : bracketName,
      bracket_data: { standings, bracketWinners, thirds: selectedThirdsIds, awards }
    };

    const { error } = await supabase.from(table).insert([payload]);
    if (error) alert(error.message);
    else alert(isAdmin ? "Official Key Saved!" : "Prediction Locked! Good Luck.");
  };
  ...
  const recalculateScores = async () => {
    ...
    try {
      ...
      for (const sub of subs) {
        ...
        Object.keys(GROUPS_DATA).forEach(gid => {
          [1, 2, 3].forEach(r => {
            if (off.standings?.[gid]?.[r] && u.standings?.[gid]?.[r] === off.standings[gid][r]) score += 2;
          });
        });

        // Bracket Points (+5, +10, +20)
        BRACKET_MAPPING.forEach(match => {
          const mid = match.id;
          const offWinnerId = off.bracketWinners?.[mid]?.id;
          if (offWinnerId && u.bracketWinners?.[mid]?.id === offWinnerId) {
            const mNum = parseInt(mid.replace('m',''));
            score += (mNum === 104 ? 20 : mNum === 103 ? 10 : 5);
          }
        });
        ...
        ['ball', 'boot', 'gloves'].forEach(key => {
            if (off.awards?.[key] && clean(u.awards?.[key]) === clean(off.awards[key])) score += 5;
        });
    ...
    } catch (err: any) { alert(err.message); } finally { setIsCalculating(false); }
  };
    ...
    return (
      <div className="min-h-screen bg-blue-600 flex items-center justify-center p-4">
        <div className="bg-white rounded-[40px] shadow-2xl max-w-lg w-full p-12 text-center">
            ...
            <p className="text-slate-400 mb-10 italic uppercase text-[10px] tracking-widest font-bold">Office Championship Arena</p>
            <input type="text" placeholder="Full Legal Name" className="w-full p-4 rounded-2xl border-2 border-slate-50 bg-slate-50 outline-none font-bold text-lg mb-4" value={bracketName} onChange={(e) => setBracketName(e.target.value)} />
            <button onClick={() => { localStorage.setItem('herd_user_name', bracketName); setIsEntryComplete(true); }} className="w-full bg-blue-600 text-white p-5 rounded-2xl font-black shadow-xl hover:bg-blue-700 transition uppercase tracking-widest text-xs">Enter Arena <ArrowRight size={18} /></button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans pb-40">
      <nav className="bg-white border-b border-slate-100 px-10 py-5 sticky top-0 z-50 flex justify-between items-center shadow-sm">
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => { const p = prompt("Pass?"); if (p === "herd2026") setIsAdmin(!isAdmin); }} className={`p-2 rounded-lg ${isAdmin ? 'bg-red-500 text-white' : 'text-slate-100'}`}><ShieldCheck size={18} /></button>
        </div>
      </nav>
      ...
      <main className="max-w-[1900px] mx-auto p-10 space-y-20">
        {view === 'bracket' ? (
          <>
            <div className="bg-blue-600 rounded-[50px] p-12 text-white shadow-2xl">
                <h2 className="text-5xl font-black italic tracking-tighter uppercase underline decoration-4 decoration-blue-300 underline-offset-8">Arena: {bracketName}</h2>
                <p className="mt-4 text-blue-100 font-medium italic">Predict the Group Stage, select 8 manual 3rd-place teams, and fill the bracket.</p>
            </div>
                  ...
                  <div className="space-y-4">
                    {g.teams.map((t: any) => (
                      <div key={t.id} className="flex items-center justify-between">
                        <div className="flex items-center gap-3"><img src={`https://flagcdn.com/w40/${t.c}.png`} className="w-5 h-3.5 object-cover rounded" alt="" /><span className="text-sm font-bold">{t.n}</span></div>
                        <div className="flex gap-1">{[1, 2, 3].map(r => (<button key={r} onClick={() => setStandings((p: any) => ({ ...p, [id]: { ...p[id], [r]: t.id } }))} className={`w-7 h-7 text-[10px] font-black rounded-lg transition ${standings[id]?.[r] === t.id ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400'}`}>{r}</button>))}</div>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <section className="bg-amber-50 border-2 border-amber-200 p-10 rounded-[3rem]">
              <h2 className="text-2xl font-black text-amber-900 uppercase italic mb-8 tracking-tighter">Promote 8 Best 3rd-Place Teams</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {Object.keys(GROUPS_DATA).map(gid => {
                  const tId = standings[gid]?.[3];
                  const t = tId ? getTeam(tId) : null;
                  if (!t) return <div key={gid} className="p-6 rounded-[2rem] border-2 border-dashed border-amber-200 opacity-40 text-center font-black text-[8px]">Group {gid} 3rd Missing</div>;
                  const isSelected = selectedThirdsIds.includes(t.id);
                  return (
                    <button key={t.id} onClick={() => toggleThirdPlaceSelection(t.id)} className={`p-6 rounded-[2rem] border-2 transition-all text-center flex flex-col items-center ${isSelected ? 'bg-blue-600 border-blue-700 shadow-xl scale-105' : 'bg-white border-amber-100'}`}>
                      <img src={`https://flagcdn.com/w40/${t.c}.png`} className="w-8 mb-2" alt="" /><span className={`text-[10px] font-black uppercase truncate w-full ${isSelected ? 'text-white' : 'text-slate-800'}`}>{t.n}</span>
            ...
            <section className="flex gap-10 overflow-x-auto pb-20 items-start">
              ...
              <BracketCol label="QF" slice={[24, 28]} resolve={resolveTeam} winners={bracketWinners} setWinners={setBracketWinners} spacing="space-y-[310px] pt-44" />
              <BracketCol label="SF" slice={[28, 30]} resolve={resolveTeam} winners={bracketWinners} setWinners={setBracketWinners} spacing="space-y-[640px] pt-[280px]" />
              <div className="flex-shrink-0 pt-[450px] space-y-40 text-center">
                  <div className="w-64 p-8 bg-amber-50 border-2 border-amber-100 rounded-[3rem]"><MatchBox m={BRACKET_MAPPING[30]} t1={resolveTeam('L101')} t2={resolveTeam('L102')} winner={bracketWinners['m103']} onPick={(w: any) => setBracketWinners((p: any) => ({ ...p, ['m103']: w }))} /></div>
                  <div className="w-80 p-12 bg-white border-[8px] border-blue-600 rounded-[80px] shadow-2xl scale-110">
                    ...
                    <div className="mt-8 border-t pt-6 text-3xl font-black text-blue-600 italic uppercase">{bracketWinners['m104']?.n || 'Champion'}</div>
                  </div>
              </div>
            </section>

            <section className="bg-indigo-600 rounded-[4rem] p-12 text-white shadow-2xl">
                <h2 className="text-3xl font-black italic tracking-tighter flex items-center gap-4 mb-10"><Star className="text-yellow-400 fill-yellow-400" /> Player Honors (+5 pts each)</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {[ { l: 'Golden Ball', k: 'ball' }, { l: 'Golden Boot', k: 'boot' }, { l: 'Golden Gloves', k: 'gloves' } ].map(a => (
                    <div key={a.k} className="bg-white/10 p-8 rounded-[2.5rem] border border-white/20">
                      <label className="text-[10px] font-black uppercase text-indigo-200 block mb-4 italic">{a.l}</label>
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[60] flex flex-col items-center gap-4">
        {isAdmin && (
          <button onClick={recalculateScores} disabled={isCalculating} className="bg-emerald-600 text-white px-10 py-3 rounded-full font-black shadow-xl hover:scale-105 transition flex items-center gap-2 text-[10px] uppercase border-4 border-white tracking-widest">
            {isCalculating ? <Loader2 className="animate-spin" size={16} /> : <Database size={16} />} Apply Scoring
          </button>
        )}
...
function LeaderboardList() {
  ...
  return (
    <div className="space-y-4">
      {list.map((u, i) => (
        <div key={i} className={`flex justify-between items-center p-8 rounded-[40px] border ${i === 0 ? 'bg-blue-600 text-white shadow-xl scale-105' : 'bg-blue-50'}`}>
          <span className="font-bold text-xl">#{i+1} {u.bracket_name}</span>
          <span className="font-black text-3xl italic">{u.points || 0} Pts</span>
        </div>
      ))}
    </div>
  );
