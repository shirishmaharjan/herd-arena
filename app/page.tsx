...
export default function HerdArenaFinalMaster() {
  ...
  // AUTO-SELECT 8 BEST 3rds BY FIFA RANK (Used for the bracket logic)
  const autoSelectedThirds = useMemo(() => {
    ...
    // Sort by FIFA rank (lower rank is better) and take top 8
    return (thirds as any[]).sort((a: any, b: any) => a.r - b.r).slice(0, 8);
  }, [standings]);

  // CORE LOGIC: Resolving who is playing in each match slot
  const resolveTeam = (slot: string) => {
    if (!slot) return null;
    
    // If winner of a previous match
    if (slot.startsWith('W')) {
        return bracketWinners['m' + slot.substring(1)];
    }
    
    // If loser of a semi-final (for 3rd place match)
    ...
    // If one of the 8 best 3rd place teams
    ...
    // If direct group qualifier (e.g., A1, B2)
    const gId = slot[0]; 
  ...
  const submitToDatabase = async () => {
    ...
    const payload = {
      ...
      bracket_data: { 
        standings, 
        bracketWinners, 
        thirds: autoSelectedThirds 
      }
    };

    const { error } = await supabase.from(table).insert([payload]);
    if (error) alert(error.message);
    else alert(isAdmin ? "Official Results Saved!" : "Prediction Locked! Good luck.");
  };
  ...
  const recalculateScores = async () => {
    ...
    for (const sub of subs) {
      ...
      // Group Points (+2 per correct rank)
      Object.keys(offData.standings || {}).forEach(gid => {
        [1,2,3].forEach(r => { 
            if(u.standings?.[gid]?.[r] === offData.standings[gid][r]) score += 2; 
        });
      });
      ...
      Object.keys(offData.bracketWinners || {}).forEach(mid => {
        if(u.bracketWinners?.[mid]?.id === offData.bracketWinners[mid]?.id) {
          if (mid === 'm103') score += 10;      // 3rd Place Match
          else if (mid === 'm104') score += 20; // Final
          else score += 5;                      // R32, R16, QF, SF
        }
      });
          ...
          <div className="bg-slate-900 p-10 text-white md:w-1/2">
             ...
             <div className="space-y-6">
                ...
                <div className="flex gap-4">
                  ...
                  <div><p className="font-bold text-lg">Knockout Wins</p><p className="text-slate-400 text-sm">+5 Points for correct winners in R32 up to Semis.</p></div>
                </div>
                <div className="flex gap-4">
                  <div className="bg-amber-500 p-2 rounded-xl h-fit"><Award size={20}/></div>
                  <div><p className="font-bold text-lg">Special Matches</p><p className="text-slate-400 text-sm">+10 for 3rd Place Winner and +20 for the World Champion.</p></div>
                </div>
                <div className="bg-red-500/10 border border-red-500/50 p-4 rounded-2xl flex gap-3">
                   <AlertCircle className="text-red-500 shrink-0" />
                   <p className="text-[11px] font-bold text-red-500 uppercase tracking-widest leading-relaxed">Disqualification: Use your real name. One submission per person.</p>
                </div>
             </div>
          </div>
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans pb-40">
      <nav className="bg-white border-b border-slate-100 px-10 py-5 sticky top-0 z-50 flex justify-between items-center shadow-sm">
        ...
        <div className="flex gap-4">
           ...
           <button onClick={() => {const p = prompt("Admin Pass?"); if(p==="herd2026") setIsAdmin(!isAdmin)}} className="text-slate-100 hover:text-slate-400 transition"><ShieldCheck size={18}/></button>
        </div>
      </nav>
          ...
          <>
            <div className="bg-blue-600 rounded-[50px] p-12 text-white shadow-2xl flex justify-between items-center relative overflow-hidden">
               <div className="relative z-10">
                  ...
                  <p className="text-blue-100 font-medium italic">Predict the entire tournament to win the Arena Trophy.</p>
               </div>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {Object.entries(GROUPS_DATA).map(([id, g]: any) => (
                  <div key={id} className="bg-white border border-slate-200 rounded-[2.5rem] p-6 shadow-sm hover:shadow-md transition">
                    <h3 className="text-[10px] font-black text-slate-400 uppercase mb-4 tracking-widest italic">Group {id}</h3>
                    <div className="space-y-4">
                      {g.teams.map((t: any) => (
                        <div key={t.id} className="flex items-center justify-between">
                          <div className="flex flex-col">
                             ...
                             <span className="text-[8px] font-bold text-slate-300 mt-1 uppercase tracking-widest">Rank {t.r}</span>
                          </div>
                          <div className="flex gap-1">
                            {[1, 2, 3].map(r => (
                              <button 
                                ...
                                className={`w-7 h-7 text-[10px] font-black rounded-lg transition ${standings[id]?.[r] === t.id ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'}`}
                              >
                                {r}
                              </button>
                            ))}
            ...
            {/* 2. AUTOMATIC 3RDS PANEL */}
            <section className="bg-amber-50 border-2 border-amber-200 p-10 rounded-[3rem] shadow-sm">
                <div className="flex justify-between items-center mb-8">
                   <h2 className="text-2xl font-black text-amber-900 uppercase italic tracking-tighter">8 Best 3rd-Place Teams (Automatic)</h2>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                   {Object.keys(GROUPS_DATA).map(gid => {
                      ...
                      return (
                        <div key={t.id} className={`p-6 rounded-[2rem] border-2 transition-all text-center flex flex-col items-center ${isQual ? 'bg-white border-blue-500 shadow-md scale-105' : 'bg-white/30 border-amber-100 opacity-50'}`}>
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[60] flex flex-col items-center gap-4">
         {isAdmin && (
            <button onClick={recalculateScores} className="bg-emerald-600 text-white px-10 py-3 rounded-full font-black shadow-xl hover:scale-105 transition flex items-center gap-2 text-[10px] uppercase border-4 border-white tracking-widest">
         ...
         <button onClick={submitToDatabase} className="bg-slate-950 text-white px-24 py-6 rounded-[2.5rem] font-black shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center gap-4 tracking-[0.2em] uppercase text-xs">
            <Save size={20}/> {isAdmin ? 'Set Official Results' : 'Lock My Prediction'}
         </button>
      </div>
    </div>
  );
...
function LeaderboardList() {
  ...
  return (
    <div className="space-y-4">
      {list.length === 0 ? (
          <p className="text-slate-400 font-bold italic">No submissions yet...</p>
      ) : list.map((u, i) => (
        <div key={i} className="flex justify-between items-center p-8 bg-blue-50 rounded-[40px] border border-blue-100">
          <span className="font-bold flex items-center gap-6 text-xl tracking-tight">
            <span className="text-blue-200 font-mono italic text-2xl">#{i+1}</span> 
            {u.bracket_name}
          </span>
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-[1.8rem] overflow-hidden shadow-sm hover:shadow-md transition duration-300">
      {[t1, t2].map((t, i) => (
        <button 
            key={i} 
            disabled={isPlaceholder(t)} 
            onClick={() => onPick(t)} 
            className={`w-full text-left p-4 flex justify-between items-center border-b last:border-0 border-slate-50 transition-all 
                ${winner?.id === t?.id && !isPlaceholder(t) ? 'bg-blue-600 text-white font-black' : 'hover:bg-blue-50'}`}
        >
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <span className={`text-[10px] font-black uppercase tracking-tight truncate 
                ${isPlaceholder(t) ? 'text-slate-300 italic font-medium' : winner?.id === t?.id ? 'text-white' : 'text-slate-800'}`}>
                {t?.n || t?.placeholder}
            </span>
          </div>
