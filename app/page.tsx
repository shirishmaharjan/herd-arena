...
import { Trophy, Check, Award, LayoutGrid, Info, ShieldCheck, Save, ArrowRight, Database, Star, AlertCircle, UserCircle } from 'lucide-react';
...
export default function HerdArenaFinalMaster() {
    ...
    return (
      <div className="min-h-screen relative flex items-center justify-center p-4 font-sans overflow-hidden bg-slate-950">
        ...
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-br from-blue-600/30 via-transparent to-black/80 mix-blend-overlay" />
        </div>

        {/* MAIN CARD (Glassmorphism UI) */}
        <div className="relative z-10 bg-white/95 backdrop-blur-2xl rounded-[50px] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.6)] max-w-4xl w-full flex flex-col md:flex-row overflow-hidden border border-white/20">
          
          {/* LEFT: ARENA RULES (HUD Style) */}
          <div className="bg-[#0f172a] p-10 text-white md:w-1/2 relative overflow-hidden">
             ...
             <div className="relative z-10">
               <h2 className="text-4xl font-black italic tracking-tighter mb-8 border-b border-blue-500 pb-4 uppercase">Arena Rules</h2>
               <div className="space-y-6">
                  <div className="flex gap-4 items-center">
                    <div className="bg-blue-600 p-2.5 rounded-xl shadow-lg shadow-blue-500/20"><Info size={20}/></div>
                    <div><p className="font-bold text-lg leading-none">Group Stage</p><p className="text-slate-400 text-xs mt-1">+2 Pts per correct rank (1,2,3)</p></div>
                  </div>
                  <div className="flex gap-4 items-center">
                    <div className="bg-indigo-600 p-2.5 rounded-xl shadow-lg shadow-indigo-500/20"><Check size={20}/></div>
                    <div><p className="font-bold text-lg leading-none">Knockout Wins</p><p className="text-slate-400 text-xs mt-1">+5 Pts per correct result</p></div>
                  </div>
                  <div className="flex gap-4 items-center">
                    <div className="bg-amber-500 p-2.5 rounded-xl shadow-lg shadow-amber-500/20"><Award size={20}/></div>
                    <div><p className="font-bold text-lg leading-none">Grand Final</p><p className="text-slate-400 text-xs mt-1">+20 Pts for correct Champ</p></div>
                  </div>
                  <div className="flex gap-4 items-center">
                    <div className="bg-emerald-500 p-2.5 rounded-xl shadow-lg shadow-emerald-500/20"><Star size={20}/></div>
                    <div><p className="font-bold text-lg leading-none">Player Honors</p><p className="text-slate-400 text-xs mt-1">+5 Pts per correct Award</p></div>
                  </div>
                  ...
                  <div className="bg-red-500/10 border border-red-500/30 p-5 rounded-3xl flex gap-4 mt-8">
                     ...
                     <p className="text-[10px] font-black text-red-400 uppercase tracking-widest leading-relaxed">
                        Submit once with real name. Duplicates are disqualified.
                     </p>
                  </div>
               </div>
             </div>
          ...
          {/* RIGHT: JOIN INTERFACE */}
          <div className="p-12 md:w-1/2 flex flex-col justify-center items-center text-center bg-white relative">
             <div className="mb-8">
                <Trophy size={64} className="text-blue-600 mx-auto drop-shadow-xl" />
                <h1 className="text-4xl font-black mt-4 italic tracking-tighter text-slate-900 uppercase">Herd <span className="text-blue-600">Arena</span></h1>
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.4em] mt-2 opacity-60">Championship 2026</p>
             </div>
             
             <div className="space-y-5 w-full max-w-xs">
                <div className="relative group">
                  <UserCircle className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-500 transition-colors" size={20} />
                  <input 
                    type="text" 
                    placeholder="Enter Full Legal Name" 
                    className="w-full pl-12 pr-4 py-5 rounded-[2rem] border-2 border-slate-50 bg-slate-50 outline-none font-bold text-md focus:bg-white focus:border-blue-600 transition-all shadow-inner" 
                    value={bracketName} 
                ...
                <button 
                  onClick={handleJoin} 
                  className="w-full bg-blue-600 text-white p-5 rounded-[2rem] font-black shadow-[0_20px_50px_-10px_rgba(37,99,235,0.5)] hover:bg-blue-700 hover:scale-[1.03] active:scale-95 transition-all flex items-center justify-center gap-3 text-[11px] tracking-[0.2em] uppercase"
                >
                  Enter Arena <ArrowRight size={18}/>
                </button>
             </div>
             <p className="mt-12 text-[8px] text-slate-300 font-black uppercase tracking-widest">Herd International IT Division</p>
          </div>
        </div>
      </div>
    );
  ...
  // --- MAIN APP (Rest of code remains exactly same as your logic) ---
  return (
    ...
    <div className="space-y-4">
      {list.map((u, i) => (
        <div key={i} className="flex justify-between items-center p-8 bg-blue-50 rounded-[40px] border border-blue-100 shadow-sm transition-all hover:scale-[1.02]">
          <span className="font-bold flex items-center gap-8 text-2xl tracking-tight"><span className="text-blue-200 font-mono italic text-3xl">#{i+1}</span> {u.bracket_name}</span>
