...
import { Trophy, Check, Award, LayoutGrid, Info, ShieldCheck, Save, ArrowRight, Database, Star, AlertCircle, UserCircle, LogOut, Search } from 'lucide-react';
...
export default function HerdArenaFinal() {
  ...
  // --- WELCOME SCREEN WITH BACKGROUND IMAGES ---
  if (!isEntryComplete) {
    return (
      <div className="min-h-screen relative flex items-center justify-center p-4 font-sans overflow-hidden">
        {/* BACKGROUND LAYERS */}
        <div className="absolute inset-0 z-0">
          <div className="absolute inset-0 bg-cover bg-center transition-transform duration-[30000ms] scale-110 animate-pulse" 
               style={{ backgroundImage: `url('https://images.unsplash.com/photo-1574629810360-7efbbe195018?q=80&w=2000')`, filter: 'brightness(0.3) saturate(1.2)' }} />
          <div className="absolute inset-0 bg-gradient-to-br from-blue-600/40 via-transparent to-black/60 mix-blend-overlay" />
        </div>

        {/* WELCOME CARD */}
        <div className="relative z-10 bg-white/95 backdrop-blur-2xl rounded-[40px] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] max-w-4xl w-full flex flex-col md:flex-row overflow-hidden border border-white/20">
          <div className="bg-slate-900 p-10 text-white md:w-1/2 relative">
             <Trophy size={200} className="absolute -bottom-10 -left-10 text-white/[0.03] -rotate-12" />
             <h2 className="text-4xl font-black italic tracking-tighter mb-8 border-b border-blue-500 pb-4">ARENA RULES</h2>
             <div className="space-y-6">
                {[
                    { i: <Info/>, l: 'Group Stage', p: '+2 Pts per correct rank (1,2,3)' },
                    { i: <Check/>, l: 'Knockout Wins', p: '+5 Pts per correct match result' },
                    { i: <Award/>, l: 'Grand Final', p: '+20 Pts for the correct Champion' },
                    { i: <Star/>, l: 'Player Honors', p: '+5 Pts per correct Golden award' }
                ].map((item, idx) => (
                    <div key={idx} className="flex gap-4 items-center">
                        <div className="bg-blue-600 p-2 rounded-xl h-fit">{item.i}</div>
                        <div><p className="font-bold text-lg leading-tight">{item.l}</p><p className="text-slate-400 text-xs mt-0.5">{item.p}</p></div>
                    </div>
                ))}
                <div className="bg-red-500/10 border border-red-500/30 p-4 rounded-2xl flex gap-3 mt-8">
                   <AlertCircle className="text-red-500 shrink-0" size={18} />
                   <p className="text-[10px] font-bold text-red-400 uppercase tracking-widest leading-relaxed">Submit only once with your full real name. Duplicates will be removed.</p>
                </div>
             </div>
          </div>
          ...
          <div className="p-12 md:w-1/2 flex flex-col justify-center items-center text-center">
             ...
             <h1 className="text-4xl font-black mb-1 italic tracking-tighter text-slate-800 uppercase">Herd <span className="text-blue-600">Arena</span></h1>
             <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.3em] mb-12">Championship 2026</p>
             <div className="space-y-5 w-full max-w-xs">
                ...
                <button onClick={handleJoin} className="w-full bg-blue-600 text-white p-5 rounded-2xl font-black shadow-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-3 text-xs tracking-widest">ENTER ARENA <ArrowRight size={18}/></button>
             </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans pb-60">
      ...
      <main className="max-w-[1900px] mx-auto p-10 space-y-20">
        {view === 'bracket' ? (
          <>
            ...
            {/* 2. AUTO-THIRDS PANEL */}
            <section className="bg-amber-50 border-2 border-amber-200 p-10 rounded-[3rem] shadow-sm relative overflow-hidden">
                <Trophy size={300} className="absolute -right-20 -top-20 text-amber-100 rotate-12" />
               ...
               <div className="flex-shrink-0 pt-[450px] space-y-40">
                  ...
                  <div className="w-80 p-12 bg-white border-[8px] border-blue-600 rounded-[80px] text-center shadow-2xl scale-110 ring-8 ring-blue-50">
                     ...
                     <div className="mt-8 border-t pt-6">
                        <p className="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest">Champion</p>
            ...
            <section className="bg-blue-600 rounded-[4rem] p-12 text-white shadow-2xl">
                <h2 className="text-3xl font-black italic tracking-tighter mb-10 flex items-center gap-4 uppercase">Player Honors (+5 pts each)</h2>
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[60] flex flex-col items-center gap-4">
         ...
         <button onClick={submitToDatabase} className="bg-slate-950 text-white px-24 py-6 rounded-[2.5rem] font-black shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center gap-4 tracking-[0.2em] uppercase text-xs">
            <Save size={20}/> {isAdmin ? 'Update Results' : 'Lock Prediction'}
         </button>
      </div>
    </div>
  );
