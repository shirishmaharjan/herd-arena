...
export default function HerdArenaFinal() {
  ...
  // --- WELCOME SCREEN WITH RULES AND BACKGROUND ---
  if (!isEntryComplete) {
    return (
      <div className="min-h-screen relative flex items-center justify-center p-4 font-sans overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          {/* LAYER 1: STADIUM IMAGE (Using Unsplash as a placeholder) */}
          <div 
            className="absolute inset-0 bg-cover bg-center animate-slow-zoom" 
            style={{ 
              backgroundImage: "url('https://images.unsplash.com/photo-1508098682722-e99c43a406b2?q=80&w=2000')", // <-- REPLACE THIS WITH YOUR LOCAL 'stadium.jpg' IF YOU HAVE IT
              filter: 'brightness(0.35) saturate(1.2)' 
            }} 
          />
          {/* LAYER 2: INTENSIFY THE BLUE COLOR & GRADIENT */}
          <div className="absolute inset-0 bg-gradient-to-br from-blue-600/40 via-transparent to-black/70" />
          <div className="absolute inset-0 bg-blue-900/20 mix-blend-overlay" />
        </div>

        {/* MAIN CARD (Glassmorphism Effect) */}
        <div className="relative z-10 bg-white/95 backdrop-blur-xl rounded-[45px] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.6)] max-w-4xl w-full flex flex-col md:flex-row overflow-hidden border border-white/20">
          
          {/* LEFT PANEL: ARENA RULES */}
          <div className="bg-[#0f172a] p-10 text-white md:w-1/2 relative overflow-hidden">
             ...
             <div className="space-y-6 relative z-10">
                ...
                <div className="bg-red-500/10 border border-red-500/30 p-4 rounded-2xl flex gap-3 mt-4">
                   ...
                   <p className="text-[10px] font-black text-red-500 uppercase tracking-widest leading-relaxed">Disqualification: Use real full names. One submission only. Duplicates will be disqualified.</p>
                </div>
             </div>
          </div>

          {/* RIGHT PANEL: JOIN INTERFACE */}
             ...
             <div className="space-y-4 w-full max-w-xs">
                <div className="relative">
                   ...
                   <input 
                     ...
                     className="w-full px-6 py-4 rounded-2xl border-2 border-slate-100 bg-slate-50/50 outline-none font-bold text-lg focus:bg-white focus:border-blue-600 transition-all shadow-inner" 
                     value={bracketName} 
  ...
  // --- MAIN APP UI (BRACKET/STANDINGS) ---
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans pb-60">
      <nav className="bg-white/95 backdrop-blur-sm border-b border-slate-100 px-10 py-5 sticky top-0 z-50 flex justify-between items-center shadow-sm">
          ...
          <>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {Object.entries(GROUPS_DATA).map(([id, g]: any) => (
                  <div key={id} className="bg-white/90 backdrop-blur-sm border border-slate-200 rounded-[2.5rem] p-6 shadow-sm hover:shadow-md transition">
            ...
            <section className="bg-amber-50/90 backdrop-blur-sm border-2 border-amber-200 p-10 rounded-[3rem] shadow-sm">
            ...
            <section className="flex gap-10 overflow-x-auto pb-20 no-scrollbar items-start">
               ...
               <div className="flex-shrink-0 pt-[450px] space-y-40">
                  <div className="w-64 p-8 bg-amber-50/90 border-2 border-amber-100 rounded-[3rem] shadow-sm text-center">
                  ...
                  <div className="w-80 p-12 bg-white/95 border-[8px] border-blue-600 rounded-[80px] text-center shadow-2xl scale-110 ring-8 ring-blue-50">
            ...
            {/* 4. PLAYER HONORS */}
            <section className="bg-indigo-600/90 rounded-[4rem] p-12 text-white shadow-2xl">
...
function LeaderboardList() {
  ...
  return (
    <div className="space-y-4">
      {list.map((u, i) => (
        <div key={i} className="flex justify-between items-center p-8 bg-blue-50/90 rounded-[40px] border border-blue-100 shadow-sm hover:shadow-md transition-all">
          <span className="font-bold flex items-center gap-6 text-2xl tracking-tight"><span className="text-blue-200 font-mono italic text-3xl">#{i+1}</span> {u.bracket_name}</span>
