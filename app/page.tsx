...
export default function HerdArenaFinalMaster() {
  ...
  const [selectedThirdsIds, setSelectedThirdsIds] = useState<string[]>([]); // New State for manual selection
  ...
  // Identify which teams are currently sitting in 3rd place in their groups
  ...
  const toggleThirdPlaceSelection = (teamId: string) => {
    setSelectedThirdsIds(prev => {
      ...
      if (prev.length >= 8) return prev; // Limit to 8 teams
      return [...prev, teamId];
    });
  };
            ...
            <section className="bg-amber-50 border-2 border-amber-200 p-10 rounded-[3rem] shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                   {Object.keys(GROUPS_DATA).map(gid => {
                      ...
                      return (
                        <button key={t.id} onClick={() => toggleThirdPlaceSelection(t.id)} className={`p-6 rounded-[2rem] border-2 transition-all text-center flex flex-col items-center relative overflow-hidden group ${isSelected ? 'bg-blue-600 border-blue-700 shadow-xl scale-105' : 'bg-white border-amber-100 hover:border-amber-300'}`}>
                           ...
                           <div className={`mt-3 flex items-center gap-1 text-[8px] font-black uppercase ${isSelected ? 'text-white' : 'text-blue-600'}`}>
                              {isSelected ? <Check size={10} strokeWidth={4}/> : <div className="w-2 h-2 rounded-full border border-blue-200" />} {isSelected ? 'Qualifying' : 'Click to Pick'}
                           </div>
                        </button>
                      );
                   })}
            ...
            <section className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-slate-900 p-10 rounded-[3rem] text-white flex flex-col justify-center">
                      ...
                      <p className="text-slate-400 text-sm mb-6 leading-relaxed">Don't just pick based on rankings. The World Cup is built on chaos. A correct Final winner is worth <span className="text-white font-black">+20 points</span>. That alone can win you the Arena!</p>
