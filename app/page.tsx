...
export default function HerdArenaFinalMaster() {
  ...
  return (
    <div className="min-h-screen relative font-sans">
      ...
      {/* 1. GLOBAL BACKGROUND IMAGE (Visible on both screens) */}
      ...
      <div className="relative z-10">
        ...
        {!isEntryComplete ? (
              ...
              <div className="bg-[#0f172a] p-10 text-white md:w-1/2 relative overflow-hidden">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-4">
                    ...
                    <div><p className="font-bold text-lg leading-none">Knockout Wins</p><p className="text-slate-400 text-xs mt-1">+5 Pts per correct winner</p></div>
                  </div>
                  <div className="flex gap-4">
                  ...
                  <div className="bg-red-500/10 border border-red-500/30 p-4 rounded-2xl flex gap-3 mt-4">
                    ...
                    <p className="text-[10px] font-bold text-red-400 uppercase tracking-widest leading-relaxed">Submit once with real name. Duplicates are disqualified.</p>
                  </div>
                </div>
              </div>

              {/* RIGHT: JOIN */}
              <div className="p-12 md:w-1/2 flex flex-col justify-center items-center text-center bg-white/80">
                ...
                <div className="space-y-4 w-full max-w-xs">
                  <input type="text" placeholder="Full Legal Name" className="w-full px-6 py-4 rounded-2xl border-2 border-slate-100 bg-slate-50/50 outline-none font-bold text-lg focus:bg-white focus:border-blue-600 transition-all" value={bracketName} onChange={(e) => setBracketName(e.target.value)} />
          ...
          /* --- MAIN APPLICATION UI --- */
          <div className="pb-40">
            <nav className="bg-white/90 backdrop-blur-md border-b border-slate-100 px-10 py-5 sticky top-0 z-50 flex justify-between items-center shadow-sm">
              ...
              <div className="flex gap-4 items-center">
                 ...
                 <button onClick={() => {const p = prompt("Admin Pass?"); if(p==="herd2026") setIsAdmin(!isAdmin)}} className="text-slate-200"><ShieldCheck size={18}/></button>
              </div>
            </nav>
            ...
            <main className="max-w-[1900px] mx-auto p-10 space-y-20">
              {view === 'bracket' ? (
                <>
                  <div className="bg-blue-600/90 backdrop-blur-md rounded-[50px] p-12 text-white shadow-2xl flex justify-between items-center">
                     <div>
                        <h2 className="text-5xl font-black italic tracking-tighter mb-2 underline decoration-blue-300 decoration-4 underline-offset-8 uppercase tracking-tight">Good Luck, {bracketName}!</h2>
                  ...
                  <section className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                      {Object.entries(GROUPS_DATA).map(([id, g]: any) => (
                        <div key={id} className="bg-white/95 backdrop-blur-sm border border-slate-200 rounded-[2.5rem] p-6 shadow-sm">
                  ...
                  {/* YELLOW PANEL: AUTOMATIC BEST 3RDS */}
                  <section className="bg-amber-50/95 backdrop-blur-md border-2 border-amber-200 p-10 rounded-[3rem] shadow-sm">
                  ...
                  <section className="flex gap-10 overflow-x-auto pb-20 no-scrollbar items-start">
                     ...
                     <div className="flex-shrink-0 pt-[450px] space-y-40">
                        ...
                        <div className="w-80 p-12 bg-white/95 border-[8px] border-blue-600 rounded-[80px] text-center shadow-2xl scale-110 ring-8 ring-blue-50/50">
                  ...
                  {/* PLAYER AWARDS */}
                  <section className="bg-indigo-600/90 backdrop-blur-md rounded-[4rem] p-12 text-white shadow-2xl">
                      <h2 className="text-3xl font-black italic tracking-tighter mb-10 flex items-center gap-4 uppercase"><Star className="text-yellow-400 fill-yellow-400"/> Player Honors (+5 pts each)</h2>
...
function LeaderboardList() {
  ...
  return (
    <div className="space-y-4">
      {list.map((u, i) => (
        <div key={i} className="flex justify-between items-center p-8 bg-blue-50/50 rounded-[40px] border border-blue-100 shadow-sm">
...
function BracketCol({ label, slice, resolve, winners, setWinners, spacing = "space-y-4" }: any) {
  return (
    <div className={`w-64 flex-shrink-0 ${spacing}`}>
      <h4 className="text-[10px] font-black text-white md:text-slate-400 uppercase text-center mb-8 tracking-widest italic">{label}</h4>
