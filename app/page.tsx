...
export default function HerdArenaFinalMaster() {
  ...
  // Resolve teams into the bracket slots
  ...
  const submitToDatabase = async () => {
    ...
    else alert(isAdmin ? "Official Key Updated!" : "Prediction Locked! Good Luck.");
  };
  ...
  const recalculateScores = async () => {
    ...
    try {
      ...
      for (const sub of subs) {
        ...
        // 1. Group Points (+2 per rank)
        Object.keys(GROUPS_DATA).forEach(gid => {
          [1, 2, 3].forEach(r => {
            const officialId = off.standings?.[gid]?.[r];
            const userId = u.standings?.[gid]?.[r];
            if (!officialId) potentialPoints += 2; // Match not decided yet
            else if (userId === officialId) currentPoints += 2; // Decided and Correct
          });
        });

        // 2. Knockout Points (Scaled)
        BRACKET_MAPPING.forEach(match => {
          const mid = match.id;
          const mNum = parseInt(mid.replace('m',''));
          const value = mNum === 104 ? 20 : mNum === 103 ? 10 : 5;

          const officialWinnerId = off.bracketWinners?.[mid]?.id;
          const userWinnerId = u.bracketWinners?.[mid]?.id;

          if (!officialWinnerId) potentialPoints += value; // Match not decided
          else if (userWinnerId === officialWinnerId) currentPoints += value; // Decided and Correct
        });

        // 3. Honors Points (+5 each)
        const clean = (s: string) => (s || '').toLowerCase().trim();
        ['ball', 'boot', 'gloves'].forEach(key => {
            const offVal = official[`golden_${key}`];
            const userVal = sub[`golden_${key}`];
            if (!offVal) potentialPoints += 5;
            else if (clean(userVal) === clean(offVal)) currentPoints += 5;
        });
      ...
      alert("Recalculation Complete! Scores reset and verified.");
        ...
        <div className="bg-white rounded-[40px] shadow-2xl max-w-4xl w-full flex flex-col md:flex-row overflow-hidden">
          <div className="bg-slate-900 p-10 text-white md:w-1/2">
            ...
            <div className="space-y-6">
               ...
               <div className="flex gap-4"><div className="bg-amber-500 p-2 rounded-xl h-fit"><Award size={20}/></div><div><p className="font-bold">The Big Ones</p><p className="text-slate-400 text-xs">+10 for 3rd Place match, +20 for the Final.</p></div></div>
            </div>
          </div>
          <div className="p-12 md:w-1/2 flex flex-col justify-center">
            ...
            <p className="text-slate-400 mb-10">Sign in to join the office championship.</p>
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans pb-40">
      <nav className="bg-white border-b border-slate-100 px-10 py-5 sticky top-0 z-50 flex justify-between items-center shadow-sm">
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => { const p = prompt("Admin Pass?"); if (p === "herd2026") setIsAdmin(!isAdmin); }} className={`p-2 rounded-lg ${isAdmin ? 'bg-red-500 text-white' : 'text-slate-100'}`}><ShieldCheck size={18} /></button>
        </div>
      </nav>
      ...
      <main className="max-w-[1900px] mx-auto p-10 space-y-20">
        {view === 'bracket' ? (
          <>
            <div className="bg-blue-600 rounded-[50px] p-12 text-white shadow-2xl flex justify-between items-center relative overflow-hidden">
              <div className="relative z-10">
                <h2 className="text-5xl font-black italic tracking-tighter mb-2 uppercase underline decoration-4 decoration-blue-300 underline-offset-8">Good Luck, {bracketName}!</h2>
                <p className="text-blue-100 font-medium italic">Fill your group ranks, then manually select your 8 best 3rd-place teams below.</p>
              </div>
            </div>
            ...
            <section className="bg-amber-50 border-2 border-amber-200 p-10 rounded-[3rem]">
              <div className="flex justify-between items-center mb-8">
                <div>
                  ...
                  <p className="text-xs text-amber-700 font-bold max-w-lg">Manually select exactly 8 teams from your 3rd place picks to advance to the R32 bracket.</p>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {Object.keys(GROUPS_DATA).map(gid => {
                  ...
                  const t = tId ? getTeam(tId) : null;
                  if (!t) return <div key={gid} className="p-6 rounded-[2rem] border-2 border-dashed border-amber-200 flex items-center justify-center opacity-40"><p className="text-[8px] font-black text-amber-400 uppercase">Pick 3rd G-{gid}</p></div>;
                  ...
                  return (
                    <button key={t.id} onClick={() => toggleThirdPlaceSelection(t.id)} className={`p-6 rounded-[2rem] border-2 transition-all text-center flex flex-col items-center group ${isSelected ? 'bg-blue-600 border-blue-700 shadow-xl scale-105' : 'bg-white border-amber-100 hover:border-amber-300'}`}>
                      ...
                      <div className={`mt-3 text-[8px] font-black uppercase ${isSelected ? 'text-white' : 'text-blue-600'}`}>{isSelected ? '✓ Advancing' : 'Add to R32'}</div>
                    </button>
                  );
                })}
              </div>
            ...
            <section className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-white p-10 rounded-[3rem] border border-slate-200">
                  <h3 className="text-lg font-black italic mb-6 flex items-center gap-2 text-blue-600 uppercase tracking-tighter"><Users /> Scouting Report: Players to Watch</h3>
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      { n: 'Vinícius Júnior', t: 'Brazil', r: "Ballon d'Or favorite" },
                      { n: 'Kylian Mbappé', t: 'France', r: 'Tournament specialist' },
                      { n: 'Jude Bellingham', t: 'England', r: 'Midfield engine' },
                      { n: 'Lamine Yamal', t: 'Spain', r: 'The generational talent' },
                      { n: 'Erling Haaland', t: 'Norway', r: 'Goal machine' },
                      { n: 'Lionel Messi', t: 'Argentina', r: 'The Final Chapter' }
                    ].map((p, i) => (
                ...
                <div className="bg-slate-900 p-10 rounded-[3rem] text-white flex flex-col justify-center">
                   <h3 className="text-2xl font-black italic mb-4 uppercase tracking-tighter">Herd Strategy</h3>
                   <p className="text-slate-400 text-sm leading-relaxed mb-6 italic">Choosing the World Champion correctly earns you <span className="text-white font-black">+20 pts</span>. It’s the single biggest point-swing in the game. Pick wisely!</p>
                   <div className="flex gap-2">
                     <span className="px-4 py-1 bg-white/5 rounded-full text-[9px] font-bold text-slate-400 border border-white/10 uppercase tracking-widest italic">#Arena2026</span>
                   </div>
                </div>
              </div>
            </section>
          ...
          <div className="max-w-2xl mx-auto bg-white p-12 rounded-[60px] shadow-2xl text-center ring-8 ring-blue-50">
            ...
            <h2 className="text-3xl font-black mb-10 italic uppercase text-slate-800 tracking-tighter">Herd Standings</h2>
            <LeaderboardList />
          </div>
        )}
      </main>
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[60] flex flex-col items-center gap-4">
        {isAdmin && (
          <button onClick={recalculateScores} disabled={isCalculating} className="bg-emerald-600 text-white px-10 py-3 rounded-full font-black shadow-xl hover:scale-105 transition flex items-center gap-2 text-[10px] uppercase border-4 border-white tracking-widest disabled:opacity-50">
            ...
            {isCalculating ? 'Recalculating...' : 'Update Leaderboard (Official)'}
          </button>
        )}
        <button onClick={submitToDatabase} className={`px-24 py-6 rounded-[2.5rem] font-black shadow-2xl hover:scale-105 active:scale-95 transition flex items-center gap-4 tracking-[0.2em] uppercase text-xs text-white ${isAdmin ? 'bg-red-600' : 'bg-slate-950'}`}>
          <Save size={20} /> {isAdmin ? 'Set Official Answer Key' : 'Lock My Prediction'}
        </button>
      </div>
    </div>
  );
      ...
      {list.map((u, i) => (
        <div key={i} className={`flex justify-between items-center p-8 rounded-[40px] border transition-all ${i === 0 ? 'bg-blue-600 border-blue-600 text-white shadow-xl scale-105' : 'bg-blue-50 border-blue-100'}`}>
          ...
          <div className="text-right">
            ...
            <p className={`text-[10px] font-black uppercase tracking-widest ${i === 0 ? 'text-blue-200' : 'text-slate-400'}`}>Points</p>
          </div>
        </div>
      ))}
    </div>
